                ...
                if (blockType !== blockTypes.AIR && blockType !== blockTypes.WATER) {
                    ...
                    if(playerAABB.intersectsBox(blockAABB)) {
                        ...
                        if (overlapSize.x < overlapSize.y && overlapSize.x < overlapSize.z) {
                            ...
                            if (playerAABB.max.y > blockAABB.max.y) {
                                onGround = true;
                                pos.y -= overlapSize.y;
...
function updatePlayer(delta) {
    ...
    handlePlayerCollision(); // Check for vertical collisions
    ...
    if (moveDirection.lengthSq() > 0) {
        ...
        controls.getObject().position.x += finalMove.x;
        controls.getObject().position.z += finalMove.z;
        handlePlayerCollision(); // Check for horizontal collisions
    }
