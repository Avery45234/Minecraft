...
function getPotentialCollidingBlocks(playerPos) {
    const playerAABB = player.aabb.clone().translate(playerPos);
    const blocks = [];
    const minX = Math.floor(playerAABB.min.x), maxX = Math.ceil(playerAABB.max.x);
    const minY = Math.floor(playerAABB.min.y), maxY = Math.ceil(playerAABB.max.y);
    const minZ = Math.floor(playerAABB.min.z), maxZ = Math.ceil(playerAABB.max.z);
...
function updatePlayer(delta) {
    ...
    // Broad phase: get all blocks we could possibly collide with
    const worldAABBs = getPotentialCollidingBlocks(pos.clone().add(motion));
    
    // Narrow phase: check and resolve collisions axis by axis
    ...
    // --- Ground Check ---
    const groundCheckAABB = player.aabb.clone().translate(pos).translate(new THREE.Vector3(0, -0.01, 0));
    player.onGround = false;
    const groundCheckBlocks = getPotentialCollidingBlocks(pos.clone().add(new THREE.Vector3(0,-0.01,0)));
    for (const blockAABB of groundCheckBlocks) {
        if (groundCheckAABB.intersectsBox(blockAABB)) {
