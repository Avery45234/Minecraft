...
// Player state
// The player is a "bean" (capsule) two blocks tall and ~0.6 wide. The camera
// sits at the eye, near the top of the body; `aabb` is expressed RELATIVE to the
// eye, so the feet are `eyeHeight` below the camera and the head is just above.
const player = {
    height: 2.0,
    radius: 0.3,
    eyeHeight: 1.7,
...
// Block selectable from the hotbar, in slot order (slots 1-7).
const HOTBAR_TYPES = [blockTypes.STONE, blockTypes.DIRT, blockTypes.GRASS, blockTypes.SAND, blockTypes.WOOD, blockTypes.LEAVES, blockTypes.WATER];
...
function init() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (match) {
            ...
            if (index < HOTBAR_TYPES.length) {
                activeBlockType = HOTBAR_TYPES[index];
                updateHotbar();
            }
        }
...
function onMouseWheel(event) {
    let currentIndex = HOTBAR_TYPES.indexOf(activeBlockType);
    if (event.deltaY < 0) { currentIndex = (currentIndex - 1 + HOTBAR_TYPES.length) % HOTBAR_TYPES.length; }
    else { currentIndex = (currentIndex + 1) % HOTBAR_TYPES.length; }
    activeBlockType = HOTBAR_TYPES[currentIndex];
    updateHotbar();
}
...
function updatePlayer(delta) {
    ...
    if (moveDirection.lengthSq() > 0) {
        ...
        finalMove.normalize().multiplyScalar(moveSpeed * delta);
    }
